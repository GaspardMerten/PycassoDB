...
def haversine(lat1, lon1, lat2, lon2, to_radians=True, earth_radius=6371):
    ...
    return (
        earth_radius * 2 * np.arcsin(np.sqrt(a))
    )  # return the value in km since the earth_radius is in km
...
class SourceComponent(Component):
    def run(self) -> None:
        ...
        for train_id in os.listdir(tempdir):
            ...
            for col in df.columns:
                if col not in ["timestamp", "lat", "lon"]:
                    ...
                    df[col] = df[col].fillna(
                        0
                    )  # TODO: May want to note the number of N/A values of each train
            ...
            df["time_difference"] = df["time_difference"].fillna(
                pd.Timedelta(seconds=0)
            )

            # Compute the distance between each tuple of the given train
            df["distance"] = (
                haversine(df["lat"].shift(), df["lon"].shift(), df["lat"], df["lon"])
                * 1000
            )  # multiplied by 1000 to have it in meters instead of kilometers
            # Replace the first distance with 0
            df.loc[df.index[0], "distance"] = 0

            # Compute the speed between each tuple of the given train
            df["speed"] = (
                df["distance"] / df["time_difference"].dt.total_seconds()
            )  # in m/s
